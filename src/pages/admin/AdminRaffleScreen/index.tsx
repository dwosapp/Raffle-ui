  ...
  if (loading) {
    return (
      <div className={classes.root}>
        <Typography variant="h3" style={{ color: 'black' }}>Loading...</Typography>
      </div>
    );
  }
      ...
      {currentRaffle ? (
        <>
          <div className={classes.headerContainer}>
            ...
            <Button onClick={revealWinners} variant="contained" style={{ color: 'white', background: '#6435C9'}}>
              Reveal winners
            </Button>
          </div>
          <Spacer height={'20px'} />
          {[...currentRaffle.entrants.entries()].map(([wallet, tickets]) => (
            <div key={wallet} className={classes.entrantRow}>
              <Typography variant="body1" style={{ color: 'black' }}>{wallet}: </Typography>
        ...
        <Typography variant="h3" style={{ color: 'black' }}>No entrants</Typography>
      )}
      <Spacer height={'50px'} />
    </div>
  );
